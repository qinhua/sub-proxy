...
echo ""
echo "运行容器："
echo "docker run -d --name sub-proxy-app --restart unless-stopped -p 3001:3001 -v /opt/sub-proxy/data:/app/server/data -v /opt/sub-proxy/upload:/app/upload -v /opt/sub-proxy/logs:/app/logs -e NODE_ENV=production -e PORT=3001 $USERNAME/$IMAGE_NAME:latest"
