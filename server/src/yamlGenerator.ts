...
export function generateVisualYaml(sub: Subscription): string {
  ...
  const { baseConfig, proxyProviders, chainProxies, ruleProviders, rules } = sub.visualConfig;
  ...
  if (!finalDoc["rule-providers"]) finalDoc["rule-providers"] = {};
  ...
  const hasDirectGroup = finalDoc["proxy-groups"].some((g: any) => g.name === DEFAULT_DIRECT_GROUP);
  if (!hasDirectGroup) {
    finalDoc["proxy-groups"].push({
  ...
  const hasForeignGroup = finalDoc["proxy-groups"].some((g: any) => g.name === DEFAULT_FOREIGN_GROUP);
  if (!hasForeignGroup) {
    finalDoc["proxy-groups"].push({
  ...
  if (rules && rules.trim().length > 0) {
    ...
    // 6.1 If no custom rules provided, add generic default rules that don't depend on chain proxies
    if (!yamlString.includes('rules:')) {
      yamlString += '\nrules:\n';
    }
    const defaultRules = [
